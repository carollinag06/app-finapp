...
export default function OnboardingStep4() {
  ...
  const options = [
    { id: 'todos_dias', label: "Todos os dias", icon: "today-outline" },
    { id: 'algumas_semana', label: "Algumas vezes por semana", icon: "apps-outline" },
    { id: 'uma_semana', label: "Uma vez por semana", icon: "calendar-clear-outline" },
    { id: 'uma_mes', label: "Uma vez por mês", icon: "moon-outline" },
  ];

  const handleContinue = () => {
    if (!selectedOption) return;
    console.log("Frequência escolhida:", selectedOption);
    router.push('/screenInicio5');
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* --- HEADER: BACK BUTTON & PROGRESS BAR 4/5 --- */}
      <View style={styles.header}>
        <View style={styles.navRow}>
          <TouchableOpacity onPress={() => router.push('/screenInicio3')} style={styles.backBtn} activeOpacity={0.7}>
            <Ionicons name="arrow-back" size={24} color="#FFF" />
          </TouchableOpacity>

          <Text style={styles.stepText}>4 <Text style={{ color: '#666' }}>DE</Text> 5</Text>
      ...
      <View style={styles.content}>
        <Text style={styles.questionText}>Com que frequência você gostaria de acompanhar suas finanças?</Text>
        <Text style={styles.subtitleText}>
          Defina o seu ritmo. Nossa IA organizará seus relatórios de acordo com a sua rotina.
        </Text>
